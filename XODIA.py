...
def getArray():   # return coordinates of rectangles for all layers
    tempArr = []
    k = 1
    for i in range(28):
        for j in range(28):
...
def drawLinesHiddInp(list1, pos, list2): # draws connections from hidden to input
    for i in range(28):
        for j in range(28):
            pygame.draw.line(display, (222, 56, 122), (list1[pos].x + 8, list1[pos].y + 15),
                             (list2[i][j].x + 5, list2[i][j].y))

def drawLinesOutHidd(list1, pos, list2): # draws connection from hidden to output
...
def createModel():
    # take a random input file from dataset
    ...
    # get the output of three layers Hidden1, Hidden2, Output Layer w.r.t input a
    features1, features2, features3 = getFeatures(a)

    # normalisation for better visualisation
    ...
    # create input image visualisation
    for i in range(28):
        for j in range(28):
            inputArr[i][j].color = (2, img[i][j], 200)
            inputArr[i][j].value = img[i][j]

    # create or set color of nodes in hidden1 and hidden2
    for i in range(100):
        # normalising upto desired scale
    ...
    # set color of nodes in output layers
    for i in range(10):
...
inputArr, hiddenArr1, hiddenArr2, outputArr = getArray()  # get position of rectangles formed
createModel()
pygame.init()
